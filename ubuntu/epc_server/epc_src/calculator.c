...
hyidx++;
if(hyidx < hymax) printf("dcs0123 = %d %d %d %d, idx = %d, indexCalibration = %d\n", dcs0, dcs1, dcs2, dcs3, index, indexCalibration);
	pixelDCS0  = dcs0 & pixelMask;
	pixelDCS1  = dcs1 & pixelMask;
...
if(hyidx < hymax) printf("dcs0123 = %d %d %d %d, arg1=%d, arg2 = %d\n", pixelDCS0, pixelDCS1, pixelDCS2, pixelDCS3, arg1, arg2);
}

int16_t calculatorGetAmpSine(){
	if (saturationCheck()){
if(hyidx < hymax) printf("SATURATION = %d\n", SATURATION);
		return SATURATION;
	}else if(adcOverflowCheck4DCS(pixelDCS0, pixelDCS1, pixelDCS2, pixelDCS3)){
if(hyidx < hymax) printf("ADC_OVERFLOW = %d\n", ADC_OVERFLOW);
		return ADC_OVERFLOW;
	}else{
if(hyidx < hymax) printf("arg1 = %d, arg2 = %d, amplitudeDivider = %d, sqrt = %f\n", arg1, arg2, amplitudeDivider, sqrt(arg1 * arg1 + arg2 * arg2));
...
if(hyidx < hymax) printf("fp_atan2 = %d\n", fp_atan2(arg1, arg2));
if(hyidx < hymax) printf("distance = %d\n", distance);

		distance = ((distance * MAX_DIST_VALUE) / (double)FP_M_2_PI) + offset + calibrationMap[indexCal] + 0.5;

if(hyidx < hymax) printf("distance = %d, MAX_DIST_VALUE = %d, FP_M_2_PI = %f, offset = %d, calibrationMap = %d\n", distance, MAX_DIST_VALUE, FP_M_2_PI,  offset, calibrationMap[indexCal]);
