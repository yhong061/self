...
//printf("storeID = %d. freq = %d, abs = %d, pidelay = %d, dualmgx = %d, verBin = %d, HorBin = %d, rowredu = %d, pn = %d, hdr = %d\n", storeID
//	, configGetModFreqMode(), configIsABS(deviceAddress), calculationIsPiDelay()
//	, calculationIsDualMGX(), readOutIsVerticalBinning(deviceAddress), readOutIsHorizontalBinning(deviceAddress)
//	, pruGetRowReduction(), calculationIsPN(),  calculationIsHDR());	
	return storeID;
}
...
//		printf("row=%d, col=%d, halve=%d\n",pruGetRowReduction(), pruGetColReduction(), pruGetNumberOfHalves());

		const int topLeftX = 4;//((i2cData[0] << 8) + i2cData[1]) / pruGetColReduction();
		const int bottomRightX = 323;//((i2cData[2] << 8) + i2cData[3]) / pruGetColReduction();
		const int topLeftY = 6;//i2cData[4];
		const int bottomRightY = 125;//i2cData[5];
		int middlePart;
		int topBottomPart;

//printf("rowReduction = %d, topLeftX = %d, bottomRightX = %d, topLeftY = %d, bottomRightY = %d \n", rowReduction, topLeftX, bottomRightX, topLeftY, bottomRightY);
		if (configGetPartType() == EPC635 || configGetPartType() == EPC503){
...
//printf("middlePart = %d, topBottomPart = %d, nRowsMax = %d, nColsMax = %d, pruGetColReduction = %d, pruGetNumberOfHalves = %d\n", middlePart, topBottomPart, nRowsMax, nColsMax, pruGetColReduction(), pruGetNumberOfHalves());
...
int calibrationLoadFPN(const int storeID, const int deviceAddress) {
	...
	if (fileUserFPN == NULL) { //if user calibration is not existing, load factory calibration
		...
		printf("[%s:%d]open : %s\n", __func__, __LINE__, pathFPNFactory);
...
int calibrationLoadOffset(const int storeID, const int deviceAddress){
	...
	if (fileUserOffset == NULL) { //if user offset is not existing, load factory offset
		...
		printf("[%s:%d] open : %s\n", __func__, __LINE__, pathOffsetFactory);
	...
	printf("[%s:%d] open : %s\n", __func__, __LINE__, pathUserOffset);
