...
int EpcCalc_ProcessCalc(int threadidx, char *databuf, unsigned int databufsize, char *calcbuf, unsigned int calcbufsize)
{
	...
	EpcCalc_apiGetDistanceAndAmplitudeSorted((EpcCalcInfo_st *)&stcalcinfo);
...
int EpcCalc_ProcessData(int threadidx, char *databuf, unsigned int databufsize)
{
	...
	if(ret < 0) {
		...
		//EpcCalc_test_saveFile(buf, size, datainfo->mSequence);
...
void *EpcCalc_ThreadRun(void *arg)  
{
	...
	DBG("thread_Calc is running. Argument was %d\n", threadidx); 
...
int EpcApi_ProcessData(char *calcbuf, unsigned int calcbufsize)
{
	...
	if(datainfo->mSequence != gSequence) {
		printf("gSequence = %d, sequence = %d\n", gSequence, datainfo->mSequence);
		gSequence = datainfo->mSequence;
	}
	if(gSequence%30 == 0) {
		gettimeofday(&tv, NULL);
		printf("S[%05d], T[%ld.%06ld] - T[%ld.%06ld]\n", gSequence, tv.tv_sec, tv.tv_usec, datainfo->mTv.tv_sec, datainfo->mTv.tv_usec);
	}
	gSequence++;
...
void *thread_Data(void *arg)  
{
	...
	printf("thread_Data is running. Argument was %d\n", my_number); 
...
void usgae(int argc, char *argv[])
{
	printf("%s framecnt filemin filemax\n", argv[0]);
	return;
}
