...
int startServer(const unsigned int addressOfDevice) {
	...
	while (1) {
		...
		else if(gSig == SIGUSR1) {
			gSig = 0;
			newsockfd = open("test", O_RDONLY);
			printf("gSig = %d, newsockfd = %d\n", gSig, newsockfd);
			sleep(1);
			printf("\n\n\n");
		}
...
printf("[%s:%d]\n", __func__, __LINE__);
	...
	apiEnableDualMGX(MGX, deviceAddress);
	apiEnablePiDelay(PI, deviceAddress);
	apiSelectMode(PN, deviceAddress);
	apiSetMinAmplitude(100);
	apiEnableAdcOverflow(1);
	//COMMANDS
#if 1
...
#endif
	if (strcmp(stringArray[0], "getDistanceSorted") == 0 && !argumentCount) {
		uint16_t *pMem = NULL;
		...
		answer = helperStringToInteger(stringArray[1]);//apiLoadConfig(helperStringToInteger(stringArray[1]), deviceAddress);
		if (answer == 1){
			//illuminationEnable();
