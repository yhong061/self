	...
	if (minAmplitude > 0) {
		for (r = 0; r < nRowsPerHalf - 1; r += 2) {
			for (c = 0; c < nCols; c++) {
				...
				amplitude = sqrt((arg1 * arg1) + (arg2 * arg2)) / 2;
				if (hysteresisUpdate(r * nCols + c, amplitude)) {
					distance = fp_atan2(arg1, arg2) + FP_M_PI;
					distance = ((distance * MAX_DIST_VALUE) /(double)FP_M_2_PI) + offset + calibrationMap[r * nCols + c] + 0.5;
				...
				if (!(configGetPartType() == EPC635 || configGetPartType() == EPC503)) {
					...
					if (hysteresisUpdate(r * nCols + nCols + c, amplitude)) {
						distance = fp_atan2(arg1, arg2) + FP_M_PI;
						distance = ((distance * MAX_DIST_VALUE) / (double)FP_M_2_PI) + offset + calibrationMap[r * nCols + nCols + c] + 0.5;
						distance = (distance + MODULO_SHIFT) % MAX_DIST_VALUE;
					} else {
...
int calcMGXPiDelayGetAmp(uint16_t **data) {
	...
	for (r = 0; r < nRowsPerHalf - 1; r += 2) {
		for (c = 0; c < nCols; c++) {
			...
			result = sqrt((arg1 * arg1) + (arg2 * arg2)) / 2;
