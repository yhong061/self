...
int camera_set_freq(int value)
{
	...
	if(ret != 0) {
		printf("Set VIDIOC_S_CTRL fail: freq\n");
	}
	else {
		printf("Set VIDIOC_S_CTRL Ok: freq\n");
	}
	return ret;
}
...
int camera_setimagefmt(enum FMT_TYPE fmttype)
{
	...
	printf("gWidth = %d, gHeight = %d, gImgsize = %d\n", gWidth, gHeight, gImgsize);
...
int camera_open(char *filename, enum FMT_TYPE fmttype)
{
    ...
    if(ret) {
        printf("[%d]fail: VIDIOC_REQBUFS\n", __LINE__);
	return -1;
    }

    printf("reqbuf.count = %d,BUFFER_COUNT = %d\n", reqbuf.count, BUFFER_COUNT);
	...
	if(ret)
	    printf("[%d]fail: VIDIOC_QBUF, errno = %d\n", __LINE__, errno);
...
int camera_close(void)
{
    ...
    if(ret) {
        printf("[%d]fail: VIDIOC_STREAMOFF, ret = %d\n", __LINE__, ret);
	//return -1;
    }
...
int camera_set_framerate(int framerate)
{
	...
	if(ret != 0) {
		printf("Set VIDIOC_S_PARM fail\n");
	}
	else {
		printf("Set VIDIOC_S_PARM Ok\n");
	}
	return ret;
}
...
int camera_get_exposure(int *value)
{
	...
	if(ret != 0) {
		printf("Set VIDIOC_G_CTRL fail\n");
	}
	else {
		*value = ctrl.value;
		printf("Set VIDIOC_G_CTRL Ok: value = %d\n", ctrl.value);
	}
	return ret;
}
...
int camera_getbuf(char *p, unsigned int size)
{
    ...
    if(ret) {
        printf("[%d]fail: VIDIOC_DQBUF: errno = %d\n", __LINE__, errno);
        return -1;
    }
...
int camera_changefmt(enum FMT_TYPE fmttype)
{
	...
	printf("reopen camera to change fmt\n");
	camera_close();
