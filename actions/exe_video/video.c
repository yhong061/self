...
{
	if(allocbuffer_type == 1)
    	camera_allocbuffer_userptr();
	else
    	camera_allocbuffer_mmap();
	return 0;
}
...
int test_framebuf(void)
{
	...
	if(savefile_frame >= frame)
			frame = savefile_frame + 2;

	printf("\n");
	DBG("get buf cnt = %d, framerate = %d\n", frame, framerate);
	if(savefile_en)
		DBG("save file en = %d, , save file frame = %d\n", savefile_en, savefile_frame);
	if(print_fps)
		DBG("print_fps = %d\n", print_fps);
    ...
    for(i=0; i<frame; i++) {
    	...
    	if(savefile_en == 1 && i == savefile_frame)  {
                sprintf(filename, "frm%d_wh%d_fmt%d_%s", i, framesize_index_i, format_index_i, CAPTURE_FILE);
...
int parse_argv(int argc, char **argv)
{
	...
	for(idx = 1; idx < argc; idx++){
		...
		else if(strcmp(argv[idx], "set_wh") == 0)	framesize_index_i = atoi(argv[++idx]);
		else if(strcmp(argv[idx], "set_format") == 0)	format_index_i = atoi(argv[++idx]);
		else if(strcmp(argv[idx], "set_fps") == 0)	{set_framerate_en = 1; set_framerate_val = atoi(argv[++idx]);}
		else if(strcmp(argv[idx], "getbuf_sec") == 0)	{getbuf_en = 1; getbuf_sec = atoi(argv[++idx]);}
		else if(strcmp(argv[idx], "getbuf_frame") == 0)	{getbuf_en = 1; getbuf_frame = atoi(argv[++idx]);}
		else if(strcmp(argv[idx], "savefile_frame") == 0)	{getbuf_en = 1; savefile_en=1; savefile_frame = atoi(argv[++idx]);}
