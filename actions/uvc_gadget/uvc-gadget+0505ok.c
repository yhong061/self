...
v4l2_dequeue_event(int fd, struct v4l2_event *event)
{
	int ret;
        ...
        DPRINTF("##:%s_%di VIDIOC_DQEVENT = 0x%x, 0x%x,0x%x\n", __func__, __LINE__, VIDIOC_DQEVENT, sizeof(struct v4l2_event), sizeof(struct timespec));
	sleep(1);
...
v4l2_subscribe_event(int fd, struct v4l2_event_subscription *sub)
{
	int ret;
        ...
        DPRINTF("##:%s_%di VIDIOC_SUBSCRIBE_EVENT = 0x%x\n", __func__, __LINE__, VIDIOC_SUBSCRIBE_EVENT);
...
uvc_gadget_fill_buffer(struct uvc_gadget *gadget, struct v4l2_buffer *outbuf)
{
	...
	printf("fill the buffer: YUYV\n");
        ...
        if (ret < 0)
         {
            printf("[%d]VIDIOC_DQBUF1 failed (%d)\n", __LINE__, ret);
            return ret;
         }
...
//    {  176, 144, { 666666, 1000000, 5000000, 0 }, },
...
uvc_gadget_events_process(struct uvc_gadget *gadget)
{
    ...
    DPRINTF("##:%s_%d v4l2_event.type = 0x%x\n", __func__, __LINE__, v4l2_event.type);
//	sleep(1);
	switch (v4l2_event.type) {
	case UVC_EVENT_CONNECT:
        DPRINTF("##:%s_%d , UVC_EVENT_CONNECT\n", __func__, __LINE__);
	case UVC_EVENT_DISCONNECT:
        DPRINTF("##:%s_%d UVC_EVENT_DISCONNECT\n", __func__, __LINE__);
		break;
	case UVC_EVENT_SETUP:

        DPRINTF("##:%s_%d UVC_EVENT_SETUP \n", __func__, __LINE__);
	...
	//sleep(1);
		...
		break;
	case UVC_EVENT_DATA:
        DPRINTF("##:%s_%d UVC_EVENT_DATA \n", __func__, __LINE__);
		uvc_gadget_events_process_data(gadget, &uvc_event->data);
		break;
	case UVC_EVENT_STREAMON:
        DPRINTF("##:%s_%d UVC_EVENT_STREAMON \n", __func__, __LINE__);
	...
	case UVC_EVENT_STREAMOFF:
        DPRINTF("##:%s_%d UVC_EVENT_STREAMOFF \n", __func__, __LINE__);
...
uvc_gadget_events_init(struct uvc_gadget *gadget)
{
	...
	uvc_gadget_fill_streaming_control(gadget, &gadget->probe, 2, 0);
	uvc_gadget_fill_streaming_control(gadget, &gadget->commit, 2, 0);
//    DPRINTF("##:%s_%d\n", __func__, __LINE__);
...
uvc_gadget_image_load(struct uvc_gadget *gadget, const char *image)
{
	...
	if (fd < 0) {
		printf("Unable to open JPEG image '%s'\n", image);
		return -1;
	}
	...
	if (gadget->imgdata == NULL) {
		printf("Unable to allocate memory for JPEG image\n");
		gadget->imgsize = 0;
		return -1;
	}
	...
	if (ret < 0)
		printf("Unable to read JPEG image '%s'\n", image);
